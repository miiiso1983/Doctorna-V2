    ...
    protected function requireAuth() {
        if (!$this->auth->check()) {
            ...
            try {
                ...
                $logLine = sprintf(
                    "[%s] AUTH FAIL path=%s sid=%s session_name=%s cookie_%s=%s cookie_DOCTORNASESSID=%s cookie_PHPSESSID=%s save_path=%s session_keys=%s\n",
                    date('c'),
                    $_SERVER['REQUEST_URI'] ?? '-',
                    session_id() ?: '-',
                    session_name(),
                    ...
                    isset($_COOKIE['DOCTORNASESSID']) ? 'present' : 'missing',
